function [d1,traceX,traceX_lb]=convex_linesearch(sys,W,b,xs,kind,d1_l,d1_u,umax)

opt_fun = @(d1) d1_to_traceX(d1,sys,W,b,xs,kind,umax);
itter = 25 %number of iterations

[traceX, d1, traceX_lb] = convexLineSearch(opt_fun,d1_l,d1_u,itter);
end

function traceX = d1_to_traceX(d1,sys,W,b,xs,kind,umax)
n_h=size(W{1},1);
dv{1} = d1*ones(n_h,1);
